...
func main() {
	...
	fmt.Printf("Enter value for accleration: ")
	araw, _ := reader.ReadString('\n')
	a, _ := strconv.ParseFloat(strings.TrimSuffix(araw, "\n"), 64)

	fmt.Printf("Enter value for initial velocity: ")
	voraw, _ := reader.ReadString('\n')
	vo, _ := strconv.ParseFloat(strings.TrimSuffix(voraw, "\n"), 64)

	fmt.Printf("Enter value for initial displacement: ")
	soraw, _ := reader.ReadString('\n')
	so, _ := strconv.ParseFloat(strings.TrimSuffix(soraw, "\n"), 64)

	fmt.Printf("Enter value for time: ")
	traw, _ := reader.ReadString('\n')
	t, _ := strconv.ParseFloat(strings.TrimSuffix(traw, "\n"), 64)
